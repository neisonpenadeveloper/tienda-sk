...
import { ShoppingBag, Search, Heart, Menu, X, Star, Home, Sparkles, Smile, ArrowRight, Package, Truck, PlusCircle, LogOut, LogIn, Upload, Trash2, Minus, Plus, ChevronDown, Share2, Check, Pencil, Wrench, Droplets, Tag, ChefHat, Shirt, Laptop, Activity, PawPrint, LayoutGrid, EyeOff, Lock } from "lucide-react";
...
// Sex Shop: vive en su propia sección (/sex-shop), con aviso de edad. Va aparte
// de ALL_CATEGORIES a propósito: así no sale en "Todos", en el buscador, en el
// carrusel ni al deslizar entre categorías de la tienda normal.
const ADULT_CAT  = { id: "adultos", label: "Sex Shop", icon: Lock };
const ADULT_PATH = "/sex-shop";
const esDeAdultos = (p) => p?.category === ADULT_CAT.id;
...
function PublishModal({ user, onClose, onPublished, defaultCategory = "hogar" }) {
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div>
              ...
              >
                ...
                <option value="adultos">🔒 Sex Shop (+18)</option>
              </select>
            </div>
...
// ─── NAVBAR ───────────────────────────────────────────────────────────────────
function Navbar({ cartCount, cartBounce, menuOpen, setMenuOpen, activeCategory, setActiveCategory, user, isAdmin, onLogin, onLogout, onPublish, onUncategorized, onCouponManager, onOrders, onCartOpen, searchQuery, setSearchQuery, onOpenMobileSearch, products, onSelectProduct, esAdultos = false }) {
            ...
            {!esAdultos && (
              <>
                ...
                <Link
                  ...
                  style={{ display: "flex", alignItems: "center", gap: "14px", padding: "10px", borderRadius: "14px", fontFamily: "var(--font-roboto), sans-serif", fontSize: "14px", fontWeight: 500, color: "#6B6560", textDecoration: "none" }}
                >
                  <div style={{ width: "40px", height: "40px", borderRadius: "11px", flexShrink: 0, background: "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center" }}>
...
const REAL_CATS = [...ALL_CATEGORIES.filter(c => c.id !== "all" && c.id !== "favoritos"), ADULT_CAT];
...
const CATEGORY_COLORS = {
  ...
  adultos:      ["#BE123C", "#881337"],
};

// ─── CATEGORY PILLS ───────────────────────────────────────────────────────────
function CategoryPills({ active, onChange, isAdmin, counts = {}, showSexShop = false }) {
    ...
    <>
      ...
      <div className="md:hidden" style={{ width: "100%", overflow: "hidden", margin: "0 -16px" }}>
        ...
        >
          ...
          {/* Entrada discreta al Sex Shop: al final y sin color */}
          {showSexShop && (
            <Link
              href={ADULT_PATH}
              style={{
                display: "flex", flexDirection: "column", alignItems: "center", gap: "6px",
                padding: "14px 18px", borderRadius: "18px", flexShrink: 0,
                border: "1.5px dashed #E0D8CC", background: "#fff", color: "#9B948E",
                fontFamily: "var(--font-roboto), sans-serif", fontSize: "11.5px", fontWeight: 500,
                whiteSpace: "nowrap", minWidth: "68px", scrollSnapAlign: "start", textDecoration: "none",
              }}
            >
      ...
      <div className="hidden md:flex items-center gap-3 flex-wrap" style={{ flexWrap: "wrap" }}>
        ...
        {showSexShop && (
          <Link
            href={ADULT_PATH}
            className="cat-pill"
            style={{ ...pillStyle(false), fontSize: "12.5px", color: "#9B948E", borderStyle: "dashed", textDecoration: "none" }}
          >
            <Lock size={13} />
            Sex Shop +18
          </Link>
...
function EmptyState({ category, searchQuery }) {
  const cat = [...ALL_CATEGORIES, ADULT_CAT].find(c => c.id === category);
  if (searchQuery?.trim()) {
    return (
...
// ─── SEX SHOP: CABECERA ───────────────────────────────────────────────────────
function SexShopHero() {
  return (
    <section style={{ background: "linear-gradient(135deg, #0F0F14 0%, #1F0A16 60%, #3B0A24 100%)", color: "#fff" }}>
      <div style={{ maxWidth: "1600px", margin: "0 auto", padding: "28px 24px", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
        <div>
          <p style={{ fontFamily: F_UI, fontSize: "11px", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "#F9A8D4", margin: "0 0 6px", display: "flex", alignItems: "center", gap: "6px" }}>
            <Lock size={12} /> Solo mayores de 18
          </p>
          <h1 style={{ fontFamily: F_DISPLAY, fontSize: "32px", fontWeight: 800, margin: "0 0 8px" }}>Sex Shop</h1>
          <p style={{ fontFamily: F_UI, fontSize: "14px", color: "#E2E8F0", margin: 0, lineHeight: 1.6 }}>
            📦 Envío en empaque discreto · 🛵 Pagas al recibir
          </p>
        </div>
        <Link
          href="/"
          style={{ fontFamily: F_UI, fontSize: "13px", fontWeight: 600, color: "#fff", textDecoration: "none", border: "1.5px solid rgba(255,255,255,0.35)", borderRadius: "20px", padding: "8px 16px", whiteSpace: "nowrap" }}
        >
          ← Volver a la tienda
        </Link>
      </div>
...
function CartDrawer({ cart, onClose, onRemove, onUpdateQty, onClearCart, user, coupons, quickBuyProduct, onClearQuickBuy }) {
  ...
  const buildDeliveryWhatsAppUrl = () => {
    ...
    if (items.some(esDeAdultos)) msg += `\n📦 *Empaque discreto, por favor*`;
    msg += `\n✅ Pago: *Contra entrega*\n\n`;
    msg += `${SEP}\n📋 *DATOS DE ENTREGA*\n${SEP}\n\n`;
...
// ─── APP ROOT ─────────────────────────────────────────────────────────────────
// zona = "tienda" (la normal, en /) o "adultos" (el Sex Shop, en /sex-shop).
export default function App({ zona = "tienda" }) {
  const esAdultos = zona === "adultos";
  ...
  // Aviso de edad del Sex Shop. null = aún sin comprobar: mientras tanto la
  // sección se tapa entera, para que no se vea nada antes de confirmar. Se
  // guarda solo para esta sesión del navegador, no para siempre.
  const [edadOk, setEdadOk] = useState(null);
  useEffect(() => {
    if (!esAdultos) return;
    let ok = false;
    try { ok = sessionStorage.getItem("sk_18") === "1"; } catch {}
    setEdadOk(ok);
  }, [esAdultos]);
  ...
  // Cada zona ve solo lo suyo: la tienda normal nunca enseña el Sex Shop, ni
  // siquiera en el buscador, y el Sex Shop solo enseña sus productos.
  const zonaProducts = displayProducts.filter(p => esAdultos === esDeAdultos(p));

  const priceMinN = priceMin ? parseInt(priceMin, 10) : null;
  const priceMaxN = priceMax ? parseInt(priceMax, 10) : null;

  const filteredProducts = zonaProducts.filter(p => {
  ...
  useEffect(() => {
    ...
    if (found && esDeAdultos(found) !== esAdultos) {
      // El enlace apunta a un producto de la otra zona: se manda allí.
      window.location.replace(`${esDeAdultos(found) ? ADULT_PATH : "/"}?p=${pid}`);
      return;
    }
    if (found) {
  ...
  const heroStats = {
    products:   zonaProducts.length,
    categories: new Set(zonaProducts.map(p => p.category)).size,
  };
  ...
  return (
    <>
      ...
      {/* Aviso de edad del Sex Shop: tapa toda la sección hasta confirmar. */}
      {esAdultos && edadOk !== true && (
        <div style={{ position: "fixed", inset: 0, zIndex: 5000, background: "#0F0F14", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
          {edadOk === false && (
            <div style={{ width: "100%", maxWidth: "380px", textAlign: "center", color: "#fff" }}>
              <div style={{ width: "64px", height: "64px", borderRadius: "50%", background: "rgba(249,168,212,0.12)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px" }}>
                <Lock size={28} color="#F9A8D4" />
              </div>
              <h2 style={{ fontFamily: F_DISPLAY, fontSize: "24px", fontWeight: 800, margin: "0 0 10px" }}>Solo para mayores de 18</h2>
              <p style={{ fontFamily: F_UI, fontSize: "14px", color: "#CBD5E1", lineHeight: 1.6, margin: "0 0 24px" }}>
                Esta sección tiene productos para adultos. Para entrar, confirma que eres mayor de edad.
              </p>
              <button
                onClick={() => { try { sessionStorage.setItem("sk_18", "1"); } catch {} setEdadOk(true); }}
                style={{ width: "100%", padding: "14px", borderRadius: "28px", border: "none", background: "linear-gradient(135deg, #BE123C, #881337)", color: "#fff", fontFamily: F_UI, fontSize: "15px", fontWeight: 700, cursor: "pointer", marginBottom: "10px" }}
              >
                Soy mayor de 18 · Entrar
              </button>
              <Link
                href="/"
                style={{ display: "block", padding: "12px", borderRadius: "28px", border: "1.5px solid rgba(255,255,255,0.25)", color: "#E2E8F0", fontFamily: F_UI, fontSize: "14px", fontWeight: 600, textDecoration: "none" }}
              >
                Salir
              </Link>
            </div>
      ...
      <div className={darkMode ? "dark" : ""} style={{ minHeight: "100vh", background: "#FAF7F4" }}>
        ...
        <Navbar
          ...
          setActiveCategory={esAdultos ? () => { window.location.href = "/"; } : handleNavCategorySelect}
          user={user}
        ...
        {showPublish && (
          <PublishModal
            user={user}
            defaultCategory={esAdultos ? ADULT_CAT.id : "hogar"}
        ...
        {esAdultos ? (
          ...
          <>
            ...
            <ProductCarousel
              products={dbProducts.filter(p => p.is_active !== false && !esDeAdultos(p)).map(p => ({ id: p.id, name: p.name, badge: p.badge, images: p.images ?? [], color: null, emoji: p.emoji }))}
        ...
        >
          ...
          <div id="productos" style={{ marginBottom: "32px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
            {!esAdultos && (
              <CategoryPills
                ...
                isAdmin={isAdmin}
                showSexShop
                counts={zonaProducts.filter(p => p.is_active !== false).reduce((acc, p) => {
          ...
          <div
            ...
            onTouchEnd={(e) => {
              ...
              if (esAdultos || Math.abs(dx) < 70 || dy > 55) return;
          ...
          {loadingProducts ? (
            ...
            <EmptyState category={esAdultos ? ADULT_CAT.id : activeCategory} searchQuery={searchQuery} />
          ) : viewMode === "list" ? (
        ...
        {(() => {
          try {
            ...
            const items = ids.map(id => zonaProducts.find(p => p.id === id)).filter(Boolean).slice(0, 6);
            ...
            <div style={{
              ...
              <div style={{ overflowY: "auto", WebkitOverflowScrolling: "touch", display: "flex", flexDirection: "column", gap: "2px" }}>
                {searchQuery.trim() === "" ? (
                  ...
                  const results = zonaProducts.filter(p => p.is_active !== false && (p.name?.toLowerCase().includes(q) || p.category?.toLowerCase().includes(q)));
